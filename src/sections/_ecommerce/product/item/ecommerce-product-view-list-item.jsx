...
export function EcommerceProductViewListItem({ product, sx, ...other }) {
  const renderLabel = () => (
    <Box sx={{ gap: 1, display: 'flex', position: 'absolute', top: 8, left: 8, zIndex: 9 }}>
  ...
  const renderContent = () => (
    <Box sx={{ gap: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
      <div>
  ...
  return (
    <Box
      sx={[
        { display: 'flex', position: 'relative', '&:hover .add-to-cart': { opacity: 1 } },
